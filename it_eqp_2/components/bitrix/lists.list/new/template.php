...
//    debug($arResult["ELEMENTS_ROWS"]);
if ($_GET['user_id']) {
//        debug($_GET['user_id']);
    $elementRows = Array();

    $db_element = CIBlockElement::GetByID($_GET['user_id']);
    if ($ar_element = $db_element->GetNext()):?>
        <h1><?=$ar_element['NAME']?></h1>
        <p>Информация о числящемся ТМЦ</p>
    <?endif;

    foreach ($arResult["ELEMENTS_ROWS"] as $ar_row) {
        $check = 0;
        $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"ACTUAL_USER"));
        if($ar_props = $db_props->Fetch()) {
            if ($ar_props['VALUE'] == $_GET['user_id']) {
                $check = 1;
            }
        }
        $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"MOL"));
        if($ar_props = $db_props->Fetch()) {
            if ($ar_props['VALUE'] == $_GET['user_id']) {
                $check = 1;
...
} elseif ($_GET['change_fact']) {
//        debug($_GET['user_id']);
    $elementRows = Array();

    $db_element = CIBlockElement::GetByID($_GET['change_fact']);
    if ($ar_element = $db_element->GetNext()):?>
        <h1><?=$ar_element['NAME']?></h1>
        <p>История изменения фактического пользователя</p>
    <?endif;

    foreach ($arResult["ELEMENTS_ROWS"] as $ar_row) {
        $check = 0;

        $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"CHANGED_ELEMENT"));
        if($ar_props = $db_props->Fetch()) {
            if ($ar_props['VALUE'] == $_GET['change_fact']) {
                $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"ACTUAL_USER"));
                $first = $db_props->Fetch();
                $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"ACTUAL_USER_STARYY"));
                $second = $db_props->Fetch();
...
} elseif ($_GET['change_mol']) {
//        debug($_GET['user_id']);
    $elementRows = Array();

    $db_element = CIBlockElement::GetByID($_GET['change_mol']);
    if ($ar_element = $db_element->GetNext()):?>
        <h1><?=$ar_element['NAME']?></h1>
        <p>История изменения МОЛ</p>
    <?endif;
    ...
    foreach ($arResult["ELEMENTS_ROWS"] as $ar_row) {
        ...
        $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"CHANGED_ELEMENT"));
        if($ar_props = $db_props->Fetch()) {
            if ($ar_props['VALUE'] == $_GET['change_mol']) {
                $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"MOL"));
                $first = $db_props->Fetch();
                $db_props = CIBlockElement::GetProperty($_GET['list_id'], $ar_row["id"], array("sort" => "asc"), Array("CODE"=>"MOL_STARYY"));
                $second = $db_props->Fetch();
                if ($first['VALUE'] != $second['VALUE']) {
                    $check = 1;
                }
...
//        debug($_GET['user_id']);
    ...
    $db_element = CIBlockElement::GetByID($_GET['change']);
    if ($ar_element = $db_element->GetNext()):?>
        <h1><?=$ar_element['NAME']?></h1>
        <p>История изменения МЦ</p>
    <?endif;
...
<div class="pagetitle-container pagetitle-align-right-container <?= $pagetitleAlignRightContainer ?>">
    ...
    <? if (in_array(1, $arGroups)  || in_array(38, $arGroups)) : ?>
        <? if ($_GET['list_id'] == 85) : ?>
            <span class="equip_btn" data-link="">Переместить</span>
        <? endif; ?>

        <span class="equip_xls_import" onclick="document.location.href=&quot;/it-equipment-2/?mode=excel&amp;amp;list_id=<?= $_GET['list_id'] ?>?mode=view&amp;list_id=<?= $_GET['list_id'] ?>&amp;section_id=0&amp;list_section_id=&amp;clear_cache=Y&amp;ncc=y&quot;" class="menu-popup-item menu-popup-no-icon "><span class="menu-popup-item-text">Экспорт в Excel</span></span>
<!--        --><?// if ($listAction) : ?>
<!--            --><?// if (in_array(1, $arGroups)) : ?>
<!--                <span id="lists-title-action" class="ui-btn ui-btn-light-border ui-btn-dropdown ui-btn-themes">-->
<!--                    --><?php //= Loc::getMessage("CT_BLL_TOOLBAR_ACTION") ?>
<!--                </span>-->
<!--            --><?// endif ?>
<!--        --><?// endif; ?>
...
<!--                <a href="--><?php //= $arResult["LIST_NEW_ELEMENT_URL"] ?><!--&mol=&fakt=&molname=&faktname=" id="lists-title-action-add" class="ui-btn-main">--><?php //= Loc::getMessage("CT_BLL_TOOLBAR_ADD") ?><!--</a>-->
...
if ($elementRows) {
    $countUser = count($elementRows);
}
//debug($countUser);

if ($_GET['user_id'] || $_GET['change_fact'] || $_GET['change_mol'] || $_GET['change']) {
    $rowCountHtml = <<<HTML
