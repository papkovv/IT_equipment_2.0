...
$users_id = array_column($Users, 'LOGIN', 'ID');
						...
						if (!$arResult['BX_isGridAjax']): ?><?
							...
							if (!empty($arResult['ROWS']) || (count($arParams['ROWS']) === 1 && $arParams['ROWS'][0]['id'] === 'template_0')) :
								...
								<?
											...
											// $fakt_name = substr(strrchr($arRow['columns']['PROPERTY_557'], "("), 1);	
											if(strlen($arRow['columns']['PROPERTY_557']) > 7){
												$fakt_name = substr(strrchr($arRow['columns']['PROPERTY_557'], ")"), 1);	
												$fakt_name = stristr($fakt_name, '<', true);
												$pattern = '~\(\K.+?(?=\))~';
												preg_match_all($pattern, $arRow['columns']['PROPERTY_557'], $arr);
												$fakt_login = $arr[0][0];
												$fakt_id = array_search($fakt_login, $users_id);

												$rsElement = CIBlockElement::GetList(
													$arOrder  = array("SORT" => "ASC"),
													$arFilter = array(
														"ACTIVE"    => "Y",
														"PROPERTY_USER" => $fakt_id,
													),
													false,
													false,
												...
												while($arElement = $rsElement->GetNext()) {
													$url_fakt = $arElement['DETAIL_PAGE_URL'];
												}
												

												$arRow['columns']['PROPERTY_557'] = '<a target="_blank" href="'. $url_fakt .'">'.$fakt_name.'</a>';
											}
											if(strlen($arRow['columns']['PROPERTY_555']) > 7){
											// $mol_name = substr(strrchr($arRow['columns']['PROPERTY_555'], "("), 1);	
											$mol_name = substr(strrchr($arRow['columns']['PROPERTY_555'], ")"), 1);	
											$mol_name = stristr($mol_name, '<', true);
											$pattern = '~\(\K.+?(?=\))~';
											// $arRow['columns']['PROPERTY_555'] = $mol_name;
											preg_match_all($pattern, $arRow['columns']['PROPERTY_555'], $arr);
											$mol_login = $arr[0][0];
											$mol_id = array_search($mol_login, $users_id);
											$rsElement = CIBlockElement::GetList(
												$arOrder  = array("SORT" => "ASC"),
												$arFilter = array(
													"ACTIVE"    => "Y",
													"PROPERTY_USER" => $mol_id,
												),
												false,
												false,
												$arSelectFields = array("ID", "NAME", "IBLOCK_ID", "CODE", "DETAIL_PAGE_URL")
											...
											while($arElement = $rsElement->GetNext()) {
												$url_mol = $arElement['DETAIL_PAGE_URL'];
											}
											
											// $eq_name = substr(strrchr($arRow['columns']['NAME'], "<"), 1);	
											$eq_name = stristr($arRow['columns']['NAME'], '>', false);
											$eq_name = substr($eq_name,1);
											$eq_name = substr($eq_name,0,-4);
											$arRow['columns']['PROPERTY_555'] = '<a target="_blank" href="'. $url_mol .'">'.$mol_name.'</a>';
											// $eq_name = $arRow['columns']['NAME'];
											}
											// if($_GET["dev"] == 'ok'){
											// 	echo '<pre>';
											// 	print_r( $arRow['columns']['PROPERTY_555']);
											// 	echo '</pre>';
											// }
								?><tr <?if($_GET["mode"] == 'view'):?> data-name="<?=$eq_name?>" data-mol="<?=$mol_id?>" data-molname="<?=$mol_name?>" data-fakt="<?=$fakt_id?>" data-faktname="<?=$fakt_name?>"<?endif;?> class="main-grid-row main-grid-row-body<?=$arRow["layout"]["row"]["class"]?> dsasd " <?=$arRow["layout"]["row"]["attributes"]?>><?
										...
										foreach ($arResult['COLUMNS'] as $id => $header):
											...
											<?
														...
														)
														{
															...
															{
//															debug($header['id']);
																if ($header['id'] != 'NAME' && $header['id'] != 'ID') {
//																	debug($header['id']);
																	$idCustom = str_replace('PROPERTY_', '', $header['id']);
																	$res = CIBlockProperty::GetByID($idCustom);
																	if($ar_code = $res->GetNext()) {};
//																	debug($ar_code['CODE']);
																}

																if ($ar_code['CODE'] == 'CHANGED_ELEMENT') {
																	$db_props = CIBlockElement::GetProperty($_GET['list_id'], $arRow["columns"]['ID'], array("sort" => "asc"), Array("CODE"=>$ar_code['CODE']));
																	if($ar_props = $db_props->Fetch()) {
																		$db_element = CIBlockElement::GetByID($ar_props['VALUE']);
																		if ($ar_element = $db_element->GetNext()):?>
																			<a target="_blank" href="?mode=edit&list_id=<?=$ar_element['IBLOCK_ID']?>&section_id=0&element_id=<?=$ar_element['ID']?>&list_section_id="><?=$ar_element['NAME']?></a>
																		<?endif;
																	}
																}
																elseif ($ar_code['CODE'] == 'ACTUAL_USER') {
																	$db_props = CIBlockElement::GetProperty($_GET['list_id'], $arRow["columns"]['ID'], array("sort" => "asc"), Array("CODE"=>$ar_code['CODE']));
																	if($ar_props = $db_props->Fetch()) {
																		$db_element = CIBlockElement::GetByID($ar_props['VALUE']);
																		if ($ar_element = $db_element->GetNext()):?>
																			<a target="_blank" href="https://testportal.avtodor-eng.ru/it-equipment-2/?mode=view&list_id=152?>&section_id=0&list_section_id=&user_id=<?=$ar_element['ID']?>"><?=$ar_element['NAME']?></a>
																		<?endif;
																	}
																} elseif ($ar_code['CODE'] == 'MOL') {
																	$db_props = CIBlockElement::GetProperty($_GET['list_id'], $arRow["columns"]['ID'], array("sort" => "asc"), Array("CODE"=>"MOL"));
																	if($ar_props = $db_props->Fetch()) {
																		$db_element = CIBlockElement::GetByID($ar_props['VALUE']);
																		if ($ar_element = $db_element->GetNext()):?>
																			<a target="_blank" href="https://testportal.avtodor-eng.ru/it-equipment-2/?mode=view&list_id=152&section_id=0&list_section_id=&user_id=<?=$ar_element['ID']?>"><?=$ar_element['NAME']?></a>
																...
																}  elseif ($ar_code['CODE'] == 'ACTUAL_USER_STARYY') {
																	$db_props = CIBlockElement::GetProperty($_GET['list_id'], $arRow["columns"]['ID'], array("sort" => "asc"), Array("CODE"=>"ACTUAL_USER_STARYY"));
																	if($ar_props = $db_props->Fetch()) {
																		$db_element = CIBlockElement::GetByID($ar_props['VALUE']);
																		if ($ar_element = $db_element->GetNext()):?>
																			<a target="_blank" href="https://testportal.avtodor-eng.ru/it-equipment-2/?mode=view&list_id=152&section_id=0&list_section_id=&user_id=<?=$ar_element['ID']?>"><?=$ar_element['NAME']?></a>
																...
																} elseif ($ar_code['CODE'] == 'MOL_STARYY') {
																	$db_props = CIBlockElement::GetProperty($_GET['list_id'], $arRow["columns"]['ID'], array("sort" => "asc"), Array("CODE"=>"MOL_STARYY"));
																	if($ar_props = $db_props->Fetch()) {
																		$db_element = CIBlockElement::GetByID($ar_props['VALUE']);
																		if ($ar_element = $db_element->GetNext()):?>
																			<a target="_blank" href="https://testportal.avtodor-eng.ru/it-equipment-2/?mode=view&list_id=152&section_id=0&list_section_id=&user_id=<?=$ar_element['ID']?>"><?=$ar_element['NAME']?></a>
																...
																} elseif ($ar_code['CODE'] == 'KTO_IZMENIL') {
																	$rsUser = CUser::GetByID($arRow["columns"][$header["id"]]);
																	$arUser = $rsUser->Fetch();
