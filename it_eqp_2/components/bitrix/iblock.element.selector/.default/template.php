...
<?if($arResult['PANEL_SELECTED_VALUES'] == 'Y'):?>
	<?if(!empty($_GET['item-id'])):?> 
		<?
	// 			   echo '<pre>';
    //   print_r( $_GET['item-id']);
    //   echo '</pre>';
			
			?>
<?if(is_array($_GET['item-id'])):
			
			
			?>
			
<div id="<?=HtmlFilter::encode($selectorId)?>_panel_selected_values">
	<div id="<?=HtmlFilter::encode($selectorId)?>_hidden_values" >
	<?foreach ($_GET['item-id'] as $key => $value) {?>
		<input type="hidden" class="hidden_values" value="<?=$value?>" data-name="<?=$_GET['item-name'][$key]?>" name="PROPERTY_602[]" id="f7pevq_selected_value_<?=$value?>">
	<?}?>
		
	</div>
	<div class="no-js" id="<?=HtmlFilter::encode($selectorId)?>_visible_values">
		<?foreach ($_GET['item-id'] as $key => $value) {?>
			<a href="/equipment/?mode=edit&list_id=85&;section_id=0&;element_id=<?=$value?>" data-lak='ok' target="_blank">[<?=$value?>] <?=$_GET['item-name'][$key]?>
		</a>
		<?}?>
	...
	<?else:?>
		<div id="<?=HtmlFilter::encode($selectorId)?>_panel_selected_values">
	<div id="<?=HtmlFilter::encode($selectorId)?>_hidden_values"><input type="hidden" class="hidden_values" value="<?=$_GET['item-id']?>" data-name="<?=$_GET['item-name']?>" name="PROPERTY_602[]" id="f7pevq_selected_value_<?=$_GET['item-id']?>"></div>
	<div class="no-js" id="<?=HtmlFilter::encode($selectorId)?>_visible_values"><a href="/equipment/?mode=edit&list_id=85&;section_id=0&;element_id=<?=$_GET['item-id']?>" data-lak='no' target="_blank">[<?=$_GET['item-id']?>] <?=$_GET['item-name']?></a></div>
	<div id="<?=HtmlFilter::encode($selectorId)?>_panel_buttons">
		<?if($arResult['POPUP'] == 'Y' && $arResult['ONLY_READ'] == "N"):?>
			<span id="<?=HtmlFilter::encode($selectorId)?>_select_button" class="ies-content-select-button">
			<?=Loc::getMessage('IEST_SELECT_ITEMS')?>
